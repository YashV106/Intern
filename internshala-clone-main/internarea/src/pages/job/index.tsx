...
const index = () => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    const salaryMin = Number(filter.salary ?? 0);
    return (jobs || []).filter((job: any) => {
      const matchesCategory = safeStr(job.category)
        .toLowerCase()
        .includes(safeStr(filter.category).toLowerCase());

      const matchesLocation = safeStr(job.location)
        .toLowerCase()
        .includes(safeStr(filter.location).toLowerCase());

      const matchesExperience = safeStr(job.Experience)
        .toLowerCase()
        .includes(safeStr(filter.experience).toLowerCase());

      const matchesWorkFromHome = !filter.workFromHome
        ? true
        : safeStr(job.location).toLowerCase().includes("remote");

      const matchesPartTime = !filter.partTime ? true : true;

      const matchesSalary = (() => {
        if (salaryMin <= 0) return true;
        const raw = safeStr(job.CTC);
        const numeric = Number(raw.replace(/[^0-9.]/g, ""));
        if (!Number.isFinite(numeric)) return true;
        return numeric >= salaryMin;
      })();
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-4">
              {(loading ? [] : filteredJobs).map((job: any) => (
                <div
                  key={job._id}
